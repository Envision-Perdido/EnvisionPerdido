...
def _load_cached_artifacts(source_name: str, domain: str, ttl_hours: int = 24) -> Optional[Dict[str, Any]]:
...
async def bootstrap_json_api_async(
    ...
    if not PLAYWRIGHT_AVAILABLE:
        raise RuntimeError("Playwright is required for bootstrap. Install with: pip install playwright")
...
def clear_bootstrap_cache(source_name: Optional[str] = None, domain: Optional[str] = None) -> None:
