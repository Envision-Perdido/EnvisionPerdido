...
# Purpose: Activate venv, load env vars, run delete_all_events.py to clear WordPress calendar
...
try {
    ...
    $logPath = Join-Path $logDir "delete_all_events_$timestamp.log"

    "[$(Get-Date -Format o)] Starting delete_all_events.py" | Tee-Object -FilePath $logPath -Append

    $python = "python"
    & $python "scripts\delete_all_events.py" 2>&1 | Tee-Object -FilePath $logPath -Append

    "[$(Get-Date -Format o)] Finished delete_all_events.py" | Tee-Object -FilePath $logPath -Append
}
catch {
    try {
        if (-not $logPath) {
            ...
            $logPath = Join-Join $fallbackDir ("delete_all_events_" + (Get-Date -Format "yyyyMMdd_HHmmss") + ".log")
        }
