...
# Purpose: Activate venv, load env vars, run fix_event_times.py to correct WordPress event times
...
try {
    ...
    $logPath = Join-Path $logDir "fix_event_times_$timestamp.log"

    "[$(Get-Date -Format o)] Starting fix_event_times.py" | Tee-Object -FilePath $logPath -Append

    $python = "python"
    & $python "scripts\fix_event_times.py" 2>&1 | Tee-Object -FilePath $logPath -Append

    "[$(Get-Date -Format o)] Finished fix_event_times.py" | Tee-Object -FilePath $logPath -Append
}
catch {
    try {
        if (-not $logPath) {
            ...
            $logPath = Join-Path $fallbackDir ("fix_event_times_" + (Get-Date -Format "yyyyMMdd_HHmmss") + ".log")
        }
