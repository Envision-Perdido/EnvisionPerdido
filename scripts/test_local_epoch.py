...
if not os.environ.get("RUN_LIVE_SITE_TESTS"):
    pytest.skip(
        "Skipping live-site epoch test (set RUN_LIVE_SITE_TESTS=1 to enable)",
...
def test_epoch_approaches():
    ...
    response = requests.get(
        f"{WP_URL}/wp-json/wp/v2/ajde_events?per_page=1", headers=headers, verify=False
    )
    ...
    print(
        f"Method 2 - Local fake epoch: {local_fake_epoch} (might display 7 PM if EventON ignores WP tz)"
    )
    ...
    print("evcal_start_time_hour: 07")
    print("evcal_start_time_min: 00")
    print("evcal_start_time_ampm: pm")
