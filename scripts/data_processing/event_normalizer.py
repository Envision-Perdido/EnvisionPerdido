...
def detect_paid_or_free(title: str = "", description: str = "", 
                        cost_text: str = "") -> Tuple[bool, PaidStatus, EventType]:
    """
    ...
    if free_found and not paid_found:
        ...
        if re.search(r'\bfree\s+(?:with|for)\s+(?:rsvp|registration)\b', 
...
def extract_cost_text(description: str = "") -> Optional[str]:
    """
...
def should_filter_brandon_styles_owa(title: str = "", location: str = "", 
...
def get_filter_reason(title: str = "", location: str = "", 
