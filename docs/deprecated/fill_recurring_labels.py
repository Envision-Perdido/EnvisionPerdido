...
def make_series_id(
    df: pd.DataFrame, id_col: str, url_col: str, title_col: str, loc_col: str
) -> pd.Series:
...
def fill_recurring_labels(df: pd.DataFrame, label_col: str = "label") -> pd.DataFrame:
    ...
    def propagate(group: pd.Series) -> pd.Series:
        ...
        if len(vals) == 1:
            ...
            return group.where(
                group.astype(str).str.replace(".0", "", 1).isin(["0", "1"]), other=label_val
            )
        return group
...
def main():
    ...
    if "series_id" not in df.columns:
        ...
        df["series_id"] = make_series_id(
            df, args.id_col, args.url_col, args.title_col, args.loc_col
        )

    # Count before
    if args.label_col in df.columns:
        labeled_before = (
            df[args.label_col].astype(str).str.replace(".0", "", 1).isin(["0", "1"]).sum()
        )
    else:
        labeled_before = 0
    ...
    series_summary = (
        ...
        .agg(
            {
                args.label_col: lambda x: (
                    x.astype(str).str.replace(".0", "", 1).isin(["0", "1"]).sum()
                ),
                "title": "first",
            }
        )
    ...
    filled_series = series_summary[series_summary["labeled_count"] > 0].sort_values(
        "labeled_count", ascending=False
    )
